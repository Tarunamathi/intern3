...
export default function LivelihoodPage() {
  ...
  const manifestPath = "/docs/livelihood/manifest.json";
  ...
  return (
    <main style={{ padding: 24 }}>
      <h2>Livelihood — Documents</h2>
      {files.length === 0 ? (
        <p>No PDFs found. Add files to <code>/public/docs/livelihood</code> and update <code>manifest.json</code>.</p>
      ) : (
        <div>
          <div style={{ margin: "12px 0", display: "flex", gap: 8 }}>
            ...
            <a href={`/docs/livelihood/${current}`} target="_blank" rel="noopener" style={{ marginLeft: 12 }}>Open in new tab</a>
            <a href={`/docs/livelihood/${current}`} download style={{ marginLeft: 8 }}>Download</a>
          </div>

          <div style={{ border: "1px solid #ddd", height: "80vh" }}>
            <iframe
              src={`/docs/livelihood/${current}`}
              title={current}
